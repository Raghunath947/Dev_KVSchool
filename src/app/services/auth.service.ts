...
export class AuthService {
  ...
  getViewLists(uid, access_level, Sclass, section, type){
    let teacherapiURL = 'http://www.cfdvs.iitb.ac.in/tempuser/kvschool/kvschool_dev_shadow/Android/viewDis.php?';
    let credentials = `uid=${uid}&access_level=${access_level}&class=${Sclass}&section=${section}&type=${type}`;
    this.apiURL = `${teacherapiURL}${credentials}`;
    console.log('API url for view posts: ', this.apiURL);
    let vData = this.http.get(this.apiURL); 
    return vData;
  }
  
  
  getPostsUI(uid,access_level){
    let teacherapiURL = 'http://www.cfdvs.iitb.ac.in/tempuser/kvschool/kvschool_dev_shadow/Android/postDiscussion.php?';
    let credentials = `uid=${uid}&access_level=${access_level}`;
    this.apiURL = `${teacherapiURL}${credentials}`;
    console.log('API url for  postsUI: ', this.apiURL);
    let vData = this.http.get(this.apiURL); 
    return vData;
  }

  sendPosts(uid, access_level, classes, sections,division, category, subject,visibility,discussion,teachers,students,smsCheck,anonymousCheck){
    let teacherapiURL = 'http://www.cfdvs.iitb.ac.in/tempuser/kvschool/kvschool_dev_shadow/Android/post_discussion_api.php?';
    let credentials = `uid=${uid}&access_level=${access_level}&classes=${classes}&modClass=${classes}&modSection=${sections}&sectionsOfClass=${sections}&modDiv=${division}&division=${division}&category=${category}&subject=${subject}&discussion=${discussion}&visibility=${visibility}&parentVisibility=${visibility}&allTeachers=${teachers}&teachers=${teachers}&students=${students}&smsCheck=${smsCheck}&anonymousCheck=${anonymousCheck}`;
  ...
  editProfile(npassword){
   ...
   let  studentName : string=localStorage.getItem('student_name');
    ...
    let credentials1 = `parentName=${parentName}&studentName=${studentName}&class=${sclass}&section=${section}&studentRollNo=${studentRollNo}&uniqueId=${uniqueId}&emailAddress=${emailAddress}&emailAddress2=${emailAddress2}&homePhone=${ homePhone}&mobileNumber=${ mobileNumber}&mobileNumber2=${ mobileNumber2}&loginId=${loginId}&password=${password}`;
   // this.apiURL = `${teacherapiURL}${credentials}`;
     
     let eapiURL = `${editpapiURL}${credentials1}`;
